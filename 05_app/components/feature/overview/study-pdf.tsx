...
export function StudyPdfDocument({ data }: { data: StudyPdfData }) {
  const cite =
    `${data.author.name || "Anonymous"} (${data.year}). ${data.title}. My Research Lab.` +
    ...
    <Document title={data.title} author={data.author.name || undefined}>
      <Page size="A4" style={s.page}>
        <Text style={s.brand} fixed>
          My Research Lab — study document
        </Text>
        ...
        <Text
          ...
          render={({ pageNumber, totalPages }) =>
            `${data.title} · page ${pageNumber} of ${totalPages} · generated by My Research Lab`
          }
        />
      </Page>
    </Document>
