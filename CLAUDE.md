...
**Implement design from the spec, never invent.** When implementing any user-facing surface in code, the work order is non-negotiable: (1) read the wireframe spec at `03_design/wireframes/<slug>.md`, (2) read `03_design/design-system/tokens.md` for the token contract, (3) look at `05_app/app/page.tsx` for reference rendering patterns (Plex Serif headlines, modular floating cards on parchment, vibrant chip treatment), (4) read the relevant section of `03_design/design-language-brief.md` v0.6 if a visual decision isn't already covered. Reference tokens by name (Tailwind utility or CSS variable), never raw hex. If a needed token is missing, add it to BOTH `03_design/design-system/tokens.md` AND `05_app/styles/tokens.css` (Light and Dark values in the same change). The design language is locked at v0.6 — do not introduce new visual decisions during implementation. Any vendor SDK import (auth, realtime, jobs, AI) lives only inside `05_app/server/adapters/<concern>.<vendor>.ts` per ADR-0007 + the lock-in inventory.

**Commit on the user's behalf in small honest units.** Per project-owner direction 2026-05-29: do `git commit` as part of finishing a unit of work, not as a separate step. One commit per meaningful unit (a scaffold landing, an ADR + its referenced files, a wireframe spec batch, a feature with its tests). Subject line is imperative <72 chars (`feat(scaffold): add tokens + ThemeProvider`); body links the relevant spec / ADR / STATUS section. **No AI/assistant attribution anywhere in the repo** (project-owner direction 2026-07-01): no `Co-Authored-By` trailer, no "generated with"/agent credit in commit messages, ADR *Deciders*, STATUS, or any doc — work is attributed to the project owner only. Never `git push` without explicit confirmation (the remote is GitHub; user controls when private state hits a hosted service). Never `git reset --hard`, `git push --force`, or rewrite shared history without explicit confirmation.
