...
**Commit on the user's behalf in small honest units.** Per project-owner direction 2026-05-29: do `git commit` as part of finishing a unit of work, not as a separate step. One commit per meaningful unit (a scaffold landing, an ADR + its referenced files, a wireframe spec batch, a feature with its tests). Subject line is imperative <72 chars (`feat(scaffold): add tokens + ThemeProvider`); body links the relevant spec / ADR / STATUS section; trailer includes `Co-Authored-By: Claude <noreply@anthropic.com>`. Never `git push` without explicit confirmation (the remote is GitHub; user controls when private state hits a hosted service). Never `git reset --hard`, `git push --force`, or rewrite shared history without explicit confirmation.
