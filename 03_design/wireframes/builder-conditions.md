...
- **Empty state (no conditions)** — copy: "No conditions yet — this study runs as a single Control group. Add a condition to compare groups."

## States

- **Default (Details, ≥1 condition):** the conditions list + Add.
- **Empty (no conditions):** empty-state copy + Add (the runtime still runs the study as implicit `control`).
