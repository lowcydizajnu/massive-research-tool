...
2. **ADR numbering / citations.**
   - **ADR-0014 is the right target** for the biometric-lifecycle amendment — *correction to my first pass:* its title is "Response data model + minimum viable conditioning," but it **is** where the PII/participant-data boundary is defined (it has a `## PII boundary` section + a V1.15 "Participants destination PII boundary" amendment, and ADR-0061 already cites it as the PII boundary). So the handoff's "ADR-0014 — PII boundary" nickname is fine; keep the amendment pointed at ADR-0014. The only nit: the filename is `0014-response-data-model-and-conditioning.md`, not `0014-pii-boundary.md` — cite the real filename in "Files to read."
