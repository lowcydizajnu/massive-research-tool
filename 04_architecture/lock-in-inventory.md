...
| **Behind an adapter** | `05_app/server/adapters/auth.ts` — `AuthAdapter` interface; no `@clerk/nextjs` imports outside `05_app/server/adapters/auth.clerk.ts` (when that file lands). |
| **Deliberate exceptions** | (none yet) |
