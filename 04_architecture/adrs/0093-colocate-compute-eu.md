...
- **Compliance:** Compute now processes request/response data in the EU (Frankfurt), matching the DB. The subprocessor disclosure (`lib/legal/subprocessors.ts`) still lists Vercel as "USA" — **updating it is an owner decision and is intentionally NOT done in this change**: that list is the single source embedded in the versioned Privacy Policy body, so editing it changes published legal text and may warrant a `CURRENT_LEGAL_VERSION` bump + user re-accept (LG3). The region move is arguably *more* protective (compute now in the EU), so a silent factual update may suffice — the owner decides. Vercel Inc. remains a US company (jurisdictional access is unchanged); this ADR changes where functions *execute*, not who the processor is.
