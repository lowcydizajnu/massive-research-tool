...
## Amendment 5 (2026-07-09) — Author ORCID on the record; OSF-side ORCID stays deferred

Prompted by the OSF "Lifecycle Open Science" alignment review (insight `01_research/insights/los-alignment-and-templates.md`, Now #3). ORCID was captured on the user profile but never propagated to the citable surface — a LOS "verifiable contributor PID" gap.

- **Shipped (record-side, fully in our control):** `getPublicStudy` + `getRecordPreview` now return `authorOrcid` (from `user.orcid`); the public record byline renders it as an `orcid.org/<id>` link. This makes the contributor PID visible + verifiable on the record itself. Feeds the machine-readable `schema.org/Person.identifier` once JSON-LD lands (Now #1).
- **NOT shipped (OSF-side ORCID push) — deliberately:** OSF only attaches an ORCID when a contributor is a **registered** OSF user (the `relationships.users → OSF user id` path); an **unregistered** contributor POST (Amendment 4) does not reliably store an ORCID. Sending an unverified `orcid` attribute would be inventing an API contract. The registered-contributor path is the **owner-deferred OSF-5 backlog item** (V1.15 #5), whose trigger is registering the OSF OAuth app — an owner action. So OSF-side ORCID remains deferred and is called out here as the reminder that memory flags on OSF work. When OSF-5 lands, thread `user.orcid` through to the registered-contributor path so it attaches on OSF too.
