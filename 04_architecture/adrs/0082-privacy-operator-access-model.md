...
- **Shipped (2026-06-29):** **full study deletion** — `studies.deleteStudy` + `server/db/delete-study.ts` hard-delete an entire study and its dependency graph in one transaction, generalizing `delete-demo.ts` for an arbitrary study (covers the RESTRICT referrers demo never populated, per an exhaustive FK audit; fixes a latent amendment-CHECK bug nulling `supersedes_version_id` + `change_summary` + `amendment_classification` together). **External-replication policy:** other-workspace replications are kept; only their lineage pointers are severed (the owner's erasure right wins over a replicator's provenance link). **Templates:** opt-in checkbox to also delete derived templates (else `TemplateExistsError`). **R2:** participant-contributed files (`resp/<responseId>/…` answer media + AI payload sidecars) are erased via the new `storage.delete()` (idempotent), collected before the DB commit, deleted best-effort after. Surfaced in the study ⋯ menu (`study-actions-menu.tsx`) with a typed-title confirm, replication warning, and template checkbox; same author-or-owner/admin guard, blocked during support access. The old inline `studies.delete` (which lacked the supersedes fix + silently deleted templates) was removed. `studies.deleteStudyPreflight` feeds the dialog's counts.
- **Known follow-up (documented, not a gap in PII erasure):** researcher-uploaded **stimuli** (`ws/<workspaceId>/…` images/audio in block configs) are NOT deleted on study delete — those keys are content-hashed / reused across studies (audio-stimulus, materials library), so deleting one study's copy could break another. Safe stimulus cleanup needs a cross-study reuse guard. Participant PII (`resp/`) — the erasure that matters — is always deleted.
- **Still committed, not yet built:** automated retention windows (the cron over `olderThanDays`); `ws/` stimulus cleanup with the reuse guard.
