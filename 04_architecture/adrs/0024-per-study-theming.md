...
## Amendment (2026-06-11) — Wave 5c: presets, page chrome, Overview auto-injection

Wave 5c shipped: presets **reddit / linkedin / youtube** + the chat trio **whatsapp / discord / imessage** (all warned + acknowledgment-gated; chat warnings are deception-sensitive), each with a `social-post` renderer override (Reddit thread, LinkedIn update, YouTube video page, chat bubbles as forwarded messages with ❤️/Forward reactions). The parked **page-level platform chrome** also shipped: `getPageFrame(presetKey)` (`components/feature/take/page-frames.tsx`) renders a vetted, DECORATIVE frame — fake top nav / masthead / chat header — above the study column for every mimicking preset; `aria-hidden` + `pointer-events-none`, so nothing that looks clickable is clickable and screen readers skip it (mitigating the dead-chrome risk). And the queued **Overview auto-injection** landed: `applyVisualContext` keeps ONE auto-managed "Visual context (auto)" methodology section in sync with the effective look (added/updated for warned presets — including customized ones via `basePresetKey` — removed when neutral); `studies.setTheme` applies it on every save, so preregistrations and the protocol-text diff capture the visual context. Preset display names moved to `PRESET_LABELS`.
