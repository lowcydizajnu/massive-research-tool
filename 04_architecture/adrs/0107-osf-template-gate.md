...
## D8 addendum (2026-07-18) — the amendment "supersedes" header now reaches structured templates too (doc-audit bug fix)

A code-grounded audit of the OSF docs (2026-07-18) surfaced a real gap, not just doc drift: the `AMENDMENT - supersedes …` provenance header (reason + auto-diff of changes since the prior registration) was **only reaching OSF for Open-Ended and the Replication Recipe**. Those two compose their own `summary` / `description` field, so the header rides inside it. The **five structured templates** (`osf-preregistration`, `social-psychology`, `secondary-data`, `as-predicted`, `osf-standard-pre-data`) instead map the researcher's answers onto OSF's fixed response keys via `toRegistrationResponses`; the header was passed to the adapter as `summaryPrefix`, which `registry.osf.ts` uses **only when no `registration_responses` are present** — so for those templates it was silently dropped, and an amended structured registration carried no visible supersedes/reason/changes provenance on OSF.

**Decision:** for a structured-template amendment, land the header in the **first long-text question** — the schema's description / narrative field — prepended so any researcher answer is preserved (new pure `prependAmendmentHeader` in `osf-schema`, unit-tested against the real captured OSF Preregistration schema). This is the same "prepend to a text field" approach the other two paths already use, made template-agnostic. If a schema has no long-text question the header stays app-side only (the amendment lineage is still recorded on the study version). The key comes from the read schema, so ADR-0107 D3 ("never emit a key we did not just read") holds.

Not verified against a live OSF amendment (that costs a permanent DOI); covered by the unit test on the pure builder + the already-verified wire shapes. The app-side lineage (`changeSummary` / `supersedesVersionId`, surfaced on the Preregister stage) was correct throughout — this only closes the OSF-side gap.
