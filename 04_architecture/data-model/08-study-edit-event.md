...
- **Coalescing:** `recordStudyEdit(experimentId, actorId, kind, summary)` updates the most recent row for the same `(experiment_id, actor_user_id, kind)` when it is **< 2 minutes old**, otherwise inserts. This keeps autosave-driven edits (slider drags, keystroke-debounced commits) from flooding the trail while preserving distinct editing sessions.
