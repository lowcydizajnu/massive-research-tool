...
| **`templateKey`** | **`"open-ended" \| "replication-recipe"`** | **Which preregistration template structures this plan and which OSF registration schema it files under. Default derived — see Invariants.** |
...
- **`templateKey` back-compat default.** A plan without a stored `templateKey` resolves to `replication-recipe` when `replicationIntent` is set, else `open-ended`. This exactly reproduces the pre-item-⑤ behaviour, where `registry-push.ts` chose the OSF schema implicitly from replication intent. Nothing needs migrating, and no existing study changes which schema it files under.
